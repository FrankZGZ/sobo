...
public class ObjectDetector : MonoBehaviour
{
    ...
    //private void Update()
    //{
    //    Debug.LogWarning("[Update] Passthrough texture not available yet.");
    //}
    ...
    private IEnumerator InferenceLoop()
    {
        while (isActiveAndEnabled)
        {
            ...
            {
                Debug.LogWarning($"TryEnsureCameraTexture {InputSize}");
                yield return null;
                continue;
            }
            Debug.LogWarning($"WaitForSeconds {InputSize}");
    ...
    {
        Debug.LogWarning($"tensorShape {InputSize}");
        ...
        var isWaiting = false;
        var downloadState = 0;
        Debug.LogWarning($"switch (downloadState) {downloadState}");
        while (true)
        {
    ...
    private bool TryEnsureCameraTexture()
    {
        ...
        if (_cameraTexture)
        {
            ...
            Debug.LogWarning("[texture] Passthrough texture not available yet.");
        }
        else
        {
